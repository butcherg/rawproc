...
LIBDIRS=-L. -L$(LIBGIMAGE)/build -L$(LIBJPEG) -L$(LIBTIFF)/libtiff/.libs -L$(LIBRAW)/lib -L$(WINLCMS)\Lib
...
$(OBJDIR)/img.o: img.cpp
	$(CXX) $(CFLAGS) $(INCLUDEDIRS) -std=c++11 -w -c img.cpp -o$@
...
$(OBJDIR)/PicProcessor.o: PicProcessor.cpp
	$(CXX) $(CFLAGS) $(WXFLAGS) -w -c PicProcessor.cpp   -o$@
...
clean:
ifeq ($(OBJDIR),win)
	rm win/*.o win/rawproc.exe win/rawprocdoc.zip
else
